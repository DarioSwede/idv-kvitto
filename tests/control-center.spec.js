...
async function setup(page, loggedIn = true) {
  await page.addInitScript(loggedIn => {
    if (loggedIn) {
      localStorage.setItem('idv-admin-connection', JSON.stringify({url: 'https://ohwalxqwtxtlldalsclj.supabase.co', anonKey: 'fake-public-key'}));
...
test('control center uses same-origin authenticated status and distinguishes copy from delivery', async ({page}) => {
  ...
  await page.route('**/api/email-status', async route => {
    ...
    expect(route.request().headers().apikey).toBe('fake-public-key');
...
test('admin login on control origin survives same-tab return without preview server', async ({page}) => {
  ...
  await page.route(/^https:\/\//, async route => {
    const path = new URL(route.request().url()).pathname;
    if (path === '/auth/v1/token') return route.fulfill({status: 200, json: {access_token: 'fake-login-token'}});
  ...
  await page.locator('#openEmailAdmin').click();
  await expect(page).toHaveURL('http://localhost:43920/admin.html');
  await page.locator('[data-view="settings"]').click();
  await page.locator('#supabaseAnonKey').fill('fake-public-key');
  await page.locator('#adminEmail').fill('staff@example.org');
  await page.locator('#adminPassword').fill('fake-password');
