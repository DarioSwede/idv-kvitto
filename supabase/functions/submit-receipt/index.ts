  ...
  try {
    ...
    try {
      ...
      for (const [index, file] of files.entries()) {
        ...
        const stampSender = pdfSafeText(shortText(`Avsändare: ${senderName} · E-post: ${senderEmail}`, 110));
        const stampReceipt = pdfSafeText(shortText(`Kvitto: ${displayName} · Belopp: ${formatAmount(displayAmount) || "—"}`, 110));
