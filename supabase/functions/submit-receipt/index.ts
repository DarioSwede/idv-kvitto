...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    return respond({ ok: true, submission_id: submission.id, submission_mode: submissionMode, delivery_sent: deliveryResult.sent, delivery_error: deliveryResult.error ?? null, copy_requested: ccSelf, copy_sent: copyResult.sent, copy_error: copyResult.error ?? null });
  } catch (error) { console.error("submit-receipt failed", error); const safeError = error instanceof Error && (error.message.includes("kunde inte omvandlas") || error.message.includes("kunde inte läsas")); const message = safeError ? error.message : "Något gick fel. Försök igen om en stund."; return respond({ error: message }, 500); }
});
