...
export async function getAdminOverview(client){
  const [loginResult,submissionsResult]=await Promise.all([
    ...
    client.from('receipt_submissions').select('id,created_at,sender_email,status,is_test').order('created_at',{ascending:false}).limit(5)
  ]);
  ...
  return {
    ...
    recent_submissions:(submissionsResult.data||[]).map(row=>({
      id:row.id,created_at:row.created_at,email:row.sender_email||null,status:row.status,is_test:Boolean(row.is_test)
    }))
  };
}
