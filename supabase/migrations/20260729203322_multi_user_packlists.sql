-- Multi-user foundation for the Sarek pack planner.
-- Existing dashboard tables remain owner-only; these four tables are the
-- deliberately multi-user surface and every row is protected by ownership RLS.
...
create table if not exists public.packing_lists (
  ...
  name        text not null default 'Sarek',
