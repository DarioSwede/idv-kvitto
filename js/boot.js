async function boot(){
  ...
  const localSubmit = new URLSearchParams(location.search).has('local-submit');
  window.__idvSubmitEndpoint = localSubmit ? '/api/test-submit' : 'https://ohwalxqwtxtlldalsclj.supabase.co/functions/v1/submit-receipt';
  ...
  html=html.replace('</head>',`<link rel="stylesheet" href="styles/app.css?v=${build}"><link rel="stylesheet" href="styles/clarity.css?v=${build}"><link rel="stylesheet" href="styles/action-layout.css?v=${build}"><link rel="stylesheet" href="styles/upload-actions.css?v=${build}"></head>`);
